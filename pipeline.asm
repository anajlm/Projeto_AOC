...
addi $t0, $s1, 0	# $t0 representa o endereço do segundo registrador (rt1)
sw  $t0, 8($s2)		# colocamos o endereco do segundo registrador (rt1) dentro da posicao 2 do vetor $s2

add $t0, $s1, 0		# $t0 representa o endereço do terceiro registrador (rd1)
...
addi $t0, $s1, 0	# $t0 representa o endereço do primeiro registrador de sub (rs2)
sw $t0, 4($s3)		# colocamos o endereco do primeiro registrador (rs2) dentro da posicao 1 do vetor $s3

addi $t0, $s1, 0	# $t0 representa o endereco do segundo registrador de sub (rt2)
sw $t0, 8($s3)		# colocamos o endereco do segundo registrador (rt2) dentro da posicao 2 do vetor $s3

addi $t0, $s1, 0  # $t0 representa o endereco do terceiro registrador de sub (rd2)
...
addi $t0, $s1, 0	# $t0 representa o endereço do primeiro registrador de and (rs3)
sw $t0, 4($s4)		# colocamos o endereco do primeiro registrador (rs3) dentro da posicao 1 do vetor $s4

addi $t0, $s1, 0	# $t0 representa o endereco do segundo registrador de and (rt3)
sw $t0, 8($s4)		# colocamos o endereco do segundo registrador (rt3) dentro da posicao 2 do vetor $s4

addi $t0, $s1, 0	# $t0 representa o endereco do terceiro registrador de and (rd3)
...
addi $t0, $s1, 0	# $t0 representa o endereço do primeiro registrador de or (rs4)
sw $t0, 4($s5)		# colocamos o endereco do primeiro registrador (rs4) dentro da posicao 1 do vetor $s5

addi $t0, $s1, 0	# $t0 representa o endereco do segundo registrador de or (rt4)
sw $t0, 8($s5)		# colocamos o endereco do segundo registrador (rt4) dentro da posicao 2 do vetor $s5

addi $t0, $s1, 0	# $t0 representa o endereco do terceiro registrador de or (rd4)
...
addi $t0, $s1, 0	# $t0 representa o endereço do primeiro registrador de addi (rs5)
sw $t0, 4($s6)		# colocamos o endereco do primeiro registrador (rs5) dentro da posicao 1 do vetor $s6

addi $t0, $s1, 0	# $t0 representa o endereco do segundo registrador de addi (rt5)
...
Execute:	
	...
	add $t1, $0, $a0	# $t4 = $v0 = RS
	add $t2, $0, $a1	# $t5 = %v1 = RT ou IMM
	add $t3, $0, $a3
	
	add $t0, $0, 001
	beq $t0, $t3, operacaosub	# Se 001 == Op/Funct da instrucao que entrou no estagio EXECUTE, entao a ALU vai fazer subtracao
	
	add $t0, $0, 010		# $t0 = OP/Funct de AND
	beq $t0, $t3, operacaoand	# Se 010 == OP/Funct da instrucao que entrou no estagio EXECUTE, entao a ALU vai fazer and
	
	add $t0, $0, 011		# $t0 = OP/Funct de OR
	beq $t0, $t3, operacaoor	# Se 011 == Op/Funct da instrucao que entrou no estagio ECECUTE, entao a ALU vai fazer or
	
	add $t0, $0, 000		# $t3 = OP/Funct de ADD
	beq $t0, $t3, operacaoadd	# Se 000 == Op/Funct da instrucao que entrou no estagio EXECUTE, entao a ALU vai fazer add
					
	add $t0, $0, 100
	beq $t0, $t3, operacaoaddi		
